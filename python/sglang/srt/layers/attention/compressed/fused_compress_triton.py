...
def _c4_decode_kernel(
    ...
    for ch in tl.static_range(4):
        ...
        tl.store(buffer_ptr + page_base + ch_off + d_offs, val, mask=d_mask)
...
def hip_compress_forward(
    *,
    ...
    if not is_decode:
        # Match compress_extend_paged: non-boundary rows are still scattered to
        # dummy out_loc=0 by the caller, so keep them at the old sentinel value.
        out.fill_(10000.0)
